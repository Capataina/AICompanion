...
The companion earns experience only from what it completes itself beside the player, never from the player's own actions; that is the owner's ruling of 14 September 2026 and the reason the award is fed from the attempt-outcome record the activity owner already emits with attribution rather than from any game event. The award is made in `../Brain/Infrastructure/Selection/OwnCurrentActivity.cs` where an attempt closes, because that is the one place its effects and its attribution meet: an attempt credited to the companion earns its family's worth per effect, a shared completion earns half, and every other attribution earns nothing. The worth per family is the tuning number in this folder's class. The total is character state, saved with `../PlayerIntegration/`, and a level is read off it on the curve in this folder. Nothing in `../Brain/` reads a level: a level changes what the companion can do and survive through the mastery tree and never what it decides, which is the ruling in the root guide.
