...
public static class BodyMotion
{
    ...
    public static BodyState Step(ITileWorld world, BodyState s, Controls c, MovementCapabilities capabilities)
    {
        if (world is IBodySimulationWorld authoritative)
            return authoritative.Simulate(s, c, capabilities);

        float left = s.Left, bottom = s.Bottom;
        BodyState controlled = MovementAbilities.ApplyControls(s, c, capabilities);
        float vx = controlled.Vx;
        float vy = controlled.Vy;
        bool grounded = controlled.OnGround;
        MobilityState mobility = controlled.Mobility;
        ...
        bool wet = world.Water(feetColumn, feetRow) || world.Lava(feetColumn, feetRow);
        int liquidKind = world.Lava(feetColumn, feetRow) ? 1 : 0;
        // A live snapshot knows the whole body volume (and honey/shimmer); retain that richer
        // observation through the tick rather than throwing it away for the feet-tile sample.
        if (wet && s.Wet)
        {
            liquidKind = s.LiquidKind;
        }
        // The game adds gravity after AI controls and before collision.
        vy = BodyPhysics.StepFall(vy, wet, liquidKind);
        float move = wet ? 0.5f : 1f;
        ...
        else if (BodyPhysics.Fits(world, nextLeft, bottom, c.FallThrough))
            left = nextLeft;
        ...
        if (vy < 0f)
        {
            ...
            if (BodyPhysics.Fits(world, left, risen, c.FallThrough))
                bottom = risen;
            else
                vy = 0f;
            return new BodyState(left, bottom, vx, vy, false, collideX, false, mobility, Capabilities: capabilities, Wet: wet, StairFall: s.StairFall || c.FallThrough, LiquidKind: liquidKind);
        }
        ...
        if (grounded)
        {
            if (!c.FallThrough && SurfaceAt(world, left, bottom))
                return new BodyState(left, bottom, vx, 0f, true, collideX, false, mobility, Capabilities: capabilities, Wet: wet, StairFall: s.StairFall || c.FallThrough, LiquidKind: liquidKind);
            if (!c.FallThrough && StepDown(world, left, bottom) is float lowered)
                return new BodyState(left, lowered, vx, 0f, true, collideX, false, mobility, Capabilities: capabilities, Wet: wet, StairFall: s.StairFall || c.FallThrough, LiquidKind: liquidKind);
        }
        ...
        while (remaining > 0f)
        {
            ...
            if (!c.FallThrough && BodyPhysics.PlatformTopCrossed(world, left, bottom, bottom + step) is float top)
                return BodyPhysics.Fits(world, left, top, c.FallThrough)
                    ? new BodyState(left, top, vx, 0f, true, collideX, false, mobility, Capabilities: capabilities, Wet: wet, StairFall: s.StairFall || c.FallThrough, LiquidKind: liquidKind)
                    : new BodyState(left, bottom, vx, 0f, true, collideX, true, mobility, Capabilities: capabilities, Wet: wet, StairFall: s.StairFall || c.FallThrough, LiquidKind: liquidKind);
            if (BodyPhysics.Fits(world, left, bottom + step, c.FallThrough))
            {
                bottom += step;
                remaining -= step;
                continue;
            }
            float? rest = BodyPhysics.RestBottom(world, left, BodyPhysics.FeetRow(bottom), bottom - 1f, c.FallThrough);
            return rest is float r && BodyPhysics.Fits(world, left, r, c.FallThrough)
                ? new BodyState(left, r, vx, 0f, true, collideX, false, mobility, Capabilities: capabilities, Wet: wet, StairFall: s.StairFall || c.FallThrough, LiquidKind: liquidKind)
                : new BodyState(left, bottom, vx, 0f, true, collideX, true, mobility, Capabilities: capabilities, Wet: wet, StairFall: s.StairFall || c.FallThrough, LiquidKind: liquidKind);
        }
        return new BodyState(left, bottom, vx, vy, false, collideX, false, mobility, Capabilities: capabilities, Wet: wet, StairFall: s.StairFall || c.FallThrough, LiquidKind: liquidKind);
    }
