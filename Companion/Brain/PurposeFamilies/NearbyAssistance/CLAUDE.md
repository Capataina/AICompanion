...
The shared interaction executor classifies a found candidate as usable and asks its subclass why a refused method is refused: lighting separates the torch setting (a prohibition), daylight on the surface (no opportunity) and an empty supply (known-unusable); pot collection separates the pot setting from a full bag. Collection publishes the classification of whichever method's raw value it publishes, and names a zero-valued known drop as usable rather than absent. One interaction is the whole job, so an attempt with a credited productive effect is complete — a placed torch without measured coverage, a broken pot with unobserved contents — while an abandoned approach, a lost jump take-off, an undelivered jump or a refused native call is a failed method and a target that stopped qualifying is invalid. A drop the collection attempt walked toward that has left the world completes it with the collector unattributed, since contact pickup is independent. Keeping company classifies as usable while the player lives and concludes every attempt as executed, never complete.
