...
Native placement establishes a resulting torch tile and the corresponding inventory debit, not that a useful dark region became brighter. The current activity checks underground/night context and native attachment/spacing; it does not yet consume measured spatial darkness. Proposal One's completion requires the coverage-aware observation described in `../../WorldObservation/CLAUDE.md`, with later light observations separated from the immediate placement outcome. A zero returned outside a native light buffer must never stand in for a dark placement opportunity.
