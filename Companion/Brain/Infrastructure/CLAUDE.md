# Infrastructure — how activities and shared behaviours get done

Nothing in this tree is a competing job. Observation, scoring, places, movement, tools, aiming, grants and recording are the machinery the seven activities and Safety/Recovery all use.

```
Infrastructure/
├─ CLAUDE.md
├─ Observation/    one account of the world per tick
├─ Selection/      utility scoring, family nomination, tunables
├─ Position/       a kind of place into a tile
├─ Movement/       one request surface and one motor
├─ Interactions/   pickaxe, axe, torch, doors, homes
├─ Aiming/         projectile trajectory solver
├─ Grants/         one packet for feet and hand; incidental pots
└─ Diagnostics/    overlay, telemetry, scenario capture
```

Weapons stay in `Companion/Weapons/`. The brain grants a free hand; the arsenal chooses who to shoot and with what.
