...
What it does not guarantee is arrival on an unfinished search. A body between a route and a replan steers straight at the goal only where the straight line is clear, and a request whose flood runs out of budget every tick over a very large free space reports pending indefinitely; the search's node limit bounds that rather than resolving it.
