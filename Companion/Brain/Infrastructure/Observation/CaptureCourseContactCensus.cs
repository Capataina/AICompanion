...
public sealed record CapturedContactEnemy(int Slot, int Generation, CapturedMeleeEnemy Shape, int Damage);
...
public static class CaptureCourseContactCensus
{
    public static CapturedContactCensus Capture(DecisionWorkBudget budget)
    {
        ...
        for (; slot < Main.maxNPCs; slot++)
        {
            ...
            enemies.Add(new(slot, HostileAttackSources.Generation(enemy), CapturedMeleeEnemy.From(enemy), enemy.damage));
        }
