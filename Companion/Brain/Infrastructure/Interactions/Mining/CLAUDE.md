...
- **The approach is the reach sense's flood over free space, and the flood is what decides whether a vein can be approached.** Every liquid is air to the orb, so a vein across a pool is approached through it like a vein across open air, and no clock reaches any of this: a row producing an undecided verdict by starving a deadline is testing nothing, and two such rows were once found doing exactly that.
