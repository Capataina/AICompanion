...
public sealed class CompanionCombat
{
    ...
    /// <summary>
    /// The flight model of the weapon the committed plan fires next, for trajectory-aware positioning; null
    /// with no plan or no uses left. The request's target is not consulted: the plan names the weapon.
    /// </summary>
    public FlightModel? ProfileFor(in ActionContext ctx, NPC? target)
    {
        AttackPlan? plan = Planner.Committed;
        if (plan == null || weapons.Count == 0)
            return null;
        AttackSegment segment = plan.Current(ctx.Senses.Tick);
        foreach (PlannedUse use in segment.Uses)
        {
            if (use.FireTick < ctx.Senses.Tick)
                continue;
            if ((uint)use.WeaponSlot < (uint)weapons.Count)
                return weapons[use.WeaponSlot].Model;
        }
        return null;
    }
