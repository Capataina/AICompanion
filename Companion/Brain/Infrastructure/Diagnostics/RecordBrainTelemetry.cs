...
public sealed class BrainTelemetry : ModSystem
{
    ...
    // 0.31.0 replaces the light scalar with the two field queries the torch decision actually reads
    // (`dark_near`, `dark_ahead`, `torch_reason`, and `light_samples`/`light_read_tick` renamed from the
    // ambient pair), adds `light_region` for the dark region the lighting activity nominated, renames
    // `reach_n`/`returnable_n` to `reach_any`/`reach_two_way` after the reach sense that now owns them,
    // and adds `reach_complete` beside them so an unfinished flood and a small one stop reading alike.
