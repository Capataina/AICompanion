...
The other half of that ruling is unbuilt and is a component rather than a weight. Hunting is meant to be how the companion explores for resources, with fighting as what happens on the way; what exists is walking at the nearest enemy. Making it exploration needs somewhere to walk *to*, which means a record of ground already visited, and the repository holds none in any form — no trail, no breadcrumb, no visited set. Approximating it with a score term would produce wandering that calls itself hunting.
