...
Prepare is required for every adapter rather than an optional hook. Score and ForecastTicks receive no world context. All ordinary candidates obey that boundary, including the transitional follow, wander and kite adapters whose responsibilities will move to the final family/shared-system structure. Environmental escape runs through SharedSafety rather than competing as a candidate.
...
The grouping is active while responsibility migration continues. Mine/chop belong to Gathering; guard/hunt and the transitional kite adapter belong to Combat; loot, pots, torches and transitional walk-with/wander belong to NearbyAssistance. Environmental escape is owned by SharedSafety outside these nominations. Current nominations distinguish positive candidates from empty families; richer unresolved-method eligibility and activation-time revalidation remain separate offer-contract work.
...
`Tools/EngineReplay/VerifyFollowRecoveryAndProtection.cs` exercises immediate guard entry against committed following and continued protection through a small retreat. Native escape fixtures separately exercise environmental response with and without ordinary offers. Those checks establish their inputs rather than every combination of threat and action factors.

The behaviour adapters remain in their current source folders during migration, and each explicitly declares its purpose family. A new activity must supply that declaration and a preparation implementation; folder names alone do not determine selection. The data flow is observation → preparation → shared evaluation → family nominations → parent choice → activity execution → position selection → shared movement → motor. Movement outcomes return as observed facts rather than a lower stage changing the parent's selected purpose.
