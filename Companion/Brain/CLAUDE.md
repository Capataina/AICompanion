...
# Planned retained-course replacement

The owner-selected [retained-course implementation plan](<../../research/proposal/Implement the Retained Course Brain.md>) specifies the next full brain change; none of its gameplay implementation is claimed here. Keep the existing movement/body and native-effect boundaries. The plan replaces stateless family-winner ordering with one course owner, concrete opportunities, conditional effect dependencies, shared computation and native receipts. Random movement, external interference, player weaving/departure and diagnostic coverage are first-version requirements. Its migration table owns the exact source moves/deletions; its internal gates form one complete implementation before a final live acceptance.
