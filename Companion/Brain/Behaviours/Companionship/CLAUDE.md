...
Prepare captures the utility value for each observation. Guard commitment refresh and clearance timers, following's geometric/sight queries and wandering's contextual value are computed there. Score receives no world context and returns that value without changing commitment or observing the player again. Entry and execution retain their separate lifecycle responsibilities.
