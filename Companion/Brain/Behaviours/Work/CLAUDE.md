...
Chopping updates discovery, approach caches and retry timers only during `Prepare`. It captures the resulting target, value and trip duration; Score and ForecastTicks read that result without advancing any timer or querying the terrain. A missing prepared candidate cannot execute a retained tree merely because the discovery cache still contains it. Mining retains its legacy discovery-in-Score path until its migration.
