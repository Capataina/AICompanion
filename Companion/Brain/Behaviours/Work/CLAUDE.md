...
Both start from the tile damage watcher in `../../WorldObservation/`, which sees every real axe or pickaxe hit through the game's own KillTile hook, and both read the player's held tool's numbers rather than owning a tool. The work itself is in `../../WorldInteractions/Chopping/` and `../../WorldInteractions/Mining/`. Mining as a whole patch: after a tile dies the next is the nearest patch tile still in reach of the current stand, and only when none is does it re-approach.
