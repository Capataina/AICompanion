...
The vector maximises damage-per-second (in encounter-life units), threat removed and player harm prevented, and minimises companion harm, push danger, time to first damage and mana. The vector keeps a company-gap slot and the weights a weight for it, and both are inert: since the owner's ruling of 25 September 2026 the search passes a gap of zero for every stand, so no stand is priced below another, or dominated by it, for being further from the player — on the play that prompted the ruling, the three stands nearest the zombies below him were each dropped as `dominated:company-gap`. The slot stays because the combat snapshot and the audit read the eight-term vector positionally; removing it is a format change of its own. Weights rise with the player's danger for prevention and time, and answer the companion's missing life and his own danger for harm. A prefix that only rolled life priced a grenade-then-bow as if the burst never marked anything; the beam threads `initialDebuffs` / `endingDebuffs` beside remaining life, in the roll's own frame.
...
`CommitAttackPlan` holds one plan. It stays while the current segment's stand is still reachable (or the body is already there), the allowance still describes it, every targeted body is still alive or was killed by the plan, no *new* hostile above the notice floor has appeared, no known hostile has grown louder than the admitted urgency plus the hold slack (a pixel closer is the plan working), no admitted body has left the place it was priced at, and the hands have made progress inside the stall window. Progress is a planned use leaving the hand or a planned hit landing on a planned generation. The clock runs from the segment's start: travel the plan ordained is not idleness. A stall defers every body the plan targeted, not only the primary. Suspension renews the window rather than consuming it.
