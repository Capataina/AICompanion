...
public sealed class ItemWeapon : CompanionWeapon
{
    ...
    /// item is swung from the orb's edge rather than its centre. The body radius is read from the one
    /// declaration of the body's box so a resized body resizes every swing with it.
    /// </summary>
    public float SwingReach => Item.width * Item.scale + BodyPhysics.Width / 2f;
        ...
        get
        {
            if (IsSwing)
                return new FlightModel(Speed: SwingReach, Motion: LearnedMotion.Straight, MaxFlightTicks: 1,
                    HitboxSize: Math.Max(1, (int)(Item.height * Item.scale)), Reach: SwingReach);
