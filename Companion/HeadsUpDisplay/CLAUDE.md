...
The notch is a glanceable health surface for the companion, not a diagnostic overlay. Clicking it opens `../ProfileCard/`; that card owns the status and optional-work controls, and opens `../Inventory/`’s bag when requested. Its location persists with `../PlayerIntegration/CompanionPlayer`. Dense brain telemetry and score rendering remain under `../Brain/Infrastructure/Diagnostics/`.
