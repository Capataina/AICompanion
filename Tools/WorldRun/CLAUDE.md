...
**Filtered means the player had an ability the companion's own declaration does not claim, not that the player left the ground.** The player's flags are compared against `MovementCapabilities.Basic`, which is the mod's declaration of the shipping kit and the same value the recorder writes into the capture's companion half; anything that carries a body through air is answered by flight and a dash by a dash. That is what makes the filter narrow on its own: the day flight lands in that record, a capture of a flying player stops being filtered and starts being graded with nothing in this folder edited. Reading the companion's half out of the capture instead would compare this build against a kit some older build declared, which is the one comparison the row must never make. The filter is whole-route rather than per-checkpoint, because nothing says *which* checkpoint needed the ability and working that out from the track is the inference the plan refuses. Swimming has no player flag to read, so it is not compared, and a player who swam somewhere the companion cannot is a gap this row does not close.
...
**`--ticks=0` plays the whole capture**, which is how the end-to-end row is taken and what the numbers in this file's findings come from. It is minutes rather than seconds once the determinism row has run it twice, at roughly two to nine milliseconds a tick depending on how much searching the window provokes. That is why verify plays a slice.
