...
- **The safety rows are a fixed set and file zero rather than vanish.** The owners are the ones `ChooseSafetyResponse` issues, and the self-test pins the set against that file in both directions, so a renamed response goes red instead of reading 0% for ever and a new response goes red instead of owning ticks no row counts.
