...
/// point of the fixed set: the movement rebuild under way on 15 September is meant to dodge on top
/// of the job rather than hand the body to a response, so the likeliest next capture is one where
/// a response no longer exists, and a row that vanished with it would read on the scoreboard as a
/// case nobody measured rather than as a share that fell to zero.
/// </summary>
public sealed class MeasureSafetyShare : IMeasure
{
    ...
    /// <summary>
    /// The owner names <c>ChooseSafetyResponse</c> issues its control requests under: the two it
    /// names as literals, and <c>combat-spacing</c>, which it issues as its own kind. The self-test's
    /// producer pin requires every one of them still to be issued there and still to be a suspending
    /// owner the grant rules classify, so a renamed response fails the self-test instead of filing a
    /// zero here for ever.
    /// </summary>
