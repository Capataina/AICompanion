...
├─ extracted-2026-09-22_10-05-56-125-tick-573.txt   where the 22 September play's combat first flickers: the session reader's parity table names it for "Playing several steps ahead" and "Weapon selection"
├─ extracted-2026-09-22_10-05-56-125-tick-829.txt   the tick the census/binder contradiction begins, named for "Knowing what it cannot do"
├─ extracted-2026-09-22_10-05-56-125-tick-1562.txt  where the last long not-fighting stretch opens, named for "Enemy selection" and "Deciding what counts as a threat"
├─ extracted-2026-09-22_10-05-56-125-tick-1950.txt  the pocket the accompanying tour lived on the face of for two hundred ticks: a five-wide pillar, a westward-opening wedge above the body, a floor two rows under it rather than directly beneath, and the way out west and down
├─ 2026-09-15_10-27-40-531-plans-orb-waits-for-route.txt  tick 10418 of the second orb play: a goal sealed past the search's node limit in a large open world, where a resumed search must begin nearer than the one before it
...
`dotnet run -p:UseAppHost=false --project Tools/NavReplay -- --extract-scenario <capture.tsv> <tick> [--size WxH] [--reason "<why this window>"]` cuts the window around the companion at a recorded tick out of the capture's terrain snapshots, with the orb's centre from `npc_px`, the destination the brain asked for, the player's feet and their trail. **Its header carries what the snapshots did not cover**, because a tile no snapshot reached is written solid — unknown is closed — and a window at low coverage is a window whose verdict is about the recording rather than about the world. Read the coverage before reading a row, and the snapshot age beside it, because the recorder writes a chunk only when it changed, so a fully covered window can describe terrain mined a minute before the tick.

**Why a window was cut belongs on its own header line and not in this file.** `--reason` appends `|| cut for <why>` after every recorded key, and the three windows below cut on 22 September 2026 were re-cut through it once the flag existed — a one-line diff each, the terrain byte-identical, which is also the cheapest re-proof of the extractor's determinism there is. Recording the reason here instead was one fact in two homes: a fixture renamed, re-cut at another tick or copied leaves this file describing something that no longer matches it and nothing notices, where a header travels with the bytes. So a paragraph here says what a *group* of windows is for and the file says why *it* was cut.

**Three of these windows were named by an instrument rather than by a person, and the instrument does not cut them.** `Tools/SessionReport`'s behaviour parity table reports, per behaviour in the README's specification, whether this capture's own checks contradicted it, and prints a ready-to-paste `--extract-scenario` command per tick it found one at — the ticks 573, 829 and 1562 files above. It prints the command instead of running it on purpose: a report is a read of one capture, and a reader that wrote a fixture as a side effect of being run would drop a different terrain window into the tree every time somebody opened a recording, named after whichever finding happened to be largest that day. A finding spanning nearly the whole session names no *place* and so is excluded from those commands, which is why the churn finding over ticks 1..2,340 produced none.
