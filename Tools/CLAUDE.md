...
**No wall-clock allowance decides a verdict in the default suite.** `EmitLedgerRows.Case` calls a reset before every case, and each instrument registers its own: the engine tier in `EngineReplay/ResetProcessState.cs`, the portable tier inline in `NavReplay/Program.cs`. The reset lifts the millisecond allowances, leaving each query's work-count limits as the only bound, and returns the statics a case can reach — the planning allowance, the search's policy switches and edge cache, the executed-route archive, the terrain log and the census — to what a fresh process holds. Both copies of every static are written, this project's and the mod assembly's, because EngineReplay compiles the movement core a second time beside the `live` alias and a reset naming one of them leaves the other holding the previous case's world.
...
Three rows needed that treatment and each announced itself by going red with its own premise assertion rather than by passing quietly, which is the fixtures being well written: the portable core's starved retained query and its millisecond stop-reason row, and the movement-failure suite's starved corridor, whose message is worth quoting because it is what a good premise sounds like — "the starved run finished its search in one tick, so it proves nothing about deadlines".
