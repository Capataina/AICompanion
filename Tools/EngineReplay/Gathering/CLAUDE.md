...
**`ore work breaks ore without excavating ordinary terrain` has never been recorded red outside the suite, and a claim that it was is the thing this paragraph exists to correct.** An earlier version of this paragraph reported it red standalone at `57db4da` with `actual native mining must reserve the tool hand`, on three runs taken during a stretch of repeated check-out-and-rebuild in one worktree. It does not reproduce, and the correction is worth keeping because the wrong reading was about to tell the next mining lane to expect a red that is not there.

What the evidence actually is, as of 22 September 2026. **Fourteen standalone greens**: three by the main thread and three by a sentinel at `57db4da` from a freshly built worktree, five by the same sentinel at `77f9e54`, and three by this lane after merging `fa4fa61` at a load average of 4.47 — plus the one that first contradicted the claim. **And every recorded red in the whole store is in-suite**: `grep -h '"case":"ore work breaks ore without excavating ordinary terrain"' Tools/Ledger/runs/*.jsonl | grep '"verdict":"fail"'` returns 52 rows, 47 at `in-suite; unbounded-allowances` and 5 at `in-suite`, and none at `alone`. That is the in-suite-only signature the root guide and `../../Ledger/CLAUDE.md` already document, and the three earlier runs were an observation nobody has explained rather than a second failure mode.

**The message is not what identifies the signature, and reading it that way is the mistake to avoid.** This fixture aborts on its first failing row, so the message names whichever assertion happened to fall first, and across those 52 reds there are ten different ones: `unchanged retained work must reuse its reach verdict` 23 times, `past the work radius the same ore must be worth nothing` 9, `raised lip must produce a native ore break` 6, `a departing player's reunion charge grows with the route home` 5, **`actual native mining must reserve the tool hand` 4**, and five others once or twice each. So that message is one of the case's in-suite messages rather than a distinct unattributed failure — and a red here is attributed by *where it ran*, in-suite against alone, never by which sentence came back.
