...
**Because every setup warms the flood, one row deliberately does not.** `AColdFloodDoesNotLeaveTheBrainResting` empties the region and runs the whole brain against an ore thirty-two tiles out — past tool reach and past the stroll band, which spans `CalmBandFar * 0.7 / 16` tiles either side of the player, so a stroll at its far edge is still five tiles short and cannot break the row by wandering into range. Getting that wrong is easy and quiet: the row first stood at fourteen tiles, inside the band, and passed.

The circle it guards is real. With a null region every work activity reads `NotYet` and offers zero, keeping company wins by default, and `Resolve`'s `Hold` branch returns before `Refresh`, so a resting tick ages the flood without growing it. What opens the circle is not a decision but a query: `SafeStrollGoal` asks `IsReturnable` of each candidate tile, and that refreshes before answering, so the first pick rejects every candidate *because* the region is null and grows the region while doing so. The row was proven to discriminate rather than assumed to — an early return in `ReachSense.Refresh` while the region is null turns it red with the feet ten pixels from spawn — and proving it needed the row hoisted to the front of the list first, because this file aborts on its first failure and the mutation reddens an earlier row.
...
Read the last column with the first. Reunion's own value is flat at its wander floor across the whole range where mining is worth anything, and mining does not decline towards a crossing — it is vetoed outright between 640 and 800, where the ore leaves the work radius. The two radii no longer overlap: by the time the player is far enough for reunion to outrank fresh work, the ore is already worth nothing. That is the same shape as the stroll band sitting outside the follow comfort, and which radius should move is a decision about how the companion feels.

What is kept and asserted is what the scene can still witness: **the remaining-work gradient inside the radius**, where a job with one hit left is worth about 3.1× a fresh one on identical geometry with the player at the same distance, asserted as a ratio so it does not encode the scores' absolute scale; and **the veto itself**, at 800, so "the band is empty" is a row rather than a sentence in a comment.
...
Every row passes on the run of 15 September 2026. The red the rebuild found here is kept as a record because it is the cleanest instance of a class that reached six other sites.
