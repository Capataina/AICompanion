...
**A gate tag on an inner row is invisible to everything that reads the ledger, and two of this suite's cases now carry one on the outer name because of it.** The plan's acceptance gates are tagged as `G13 …`, `G10 …`, `G08 …` prefixes on `RunOneRow.Case` names — 87 rows across ten gates — and `RunOneRow` never files a row: it prints, and folds a failure into the *enclosing* `EmitLedgerRows.Case`. So the ledger's unit carries no gate identifier, `sh Tools/run-case.sh "G13"` exits 2 with "no case matched", and no run file, scoreboard or grep over `Tools/Ledger/runs/` can answer which gates have rows. The plan's section 9 table stays the only record of coverage, and it is hand-maintained prose, which is the artefact the gate work exists because nobody could check.

Two outer cases carry the prefix as of 22 September 2026 — `G13 an encounter suppresses work …` and `G10 no domain recomputes …` — and **that is a fix for two cases rather than for the class**. Giving the other 87 rows one means either renaming the outer cases that contain them, which the scoreboard reads as `gone` plus `new` for each and which costs a baseline per case, or filing the inner names as ledger rows, which changes the granularity of every one of those files and is the migration the verification plan already wants. The two here were new on their branch with no baseline, so the prefix cost nothing; anything wider is a decision about the case set rather than an edit. **A row belonging to no gate carries no prefix rather than the nearest one** — the vein rows in `Gathering/VerifyVeinRemainingWork.cs` were first tagged `G08`, which is *repairs across domains* and is not what they assert, and the tag came off rather than being traded for another.

**Case granularity here is mixed, and which kind a row is decides what a red tells you.** The older families are one case per fixture *file*, which is the granularity reachable without restructuring the fixture and is enough for per-case selection, rerun-red and a scoreboard that names what moved. The newer flags are one case per assertion, through `RunOneRow.Case` — `--volley-learning`, `--flight-laws`, `--simulated-uses`, `--weapon-outcome-credit`, `--combat-activity`, `--retained-course-recording` and `--retained-course-combat` are all written that way, and each names its own row. The verification plan wants the second everywhere and the migration is partial, so a red on a file-grained case hides its own later rows (below) while a red on an assertion-grained one does not.

**Inside one fixture the abort is still real, and it is the single most misleading thing about reading a red here.** A fixture runs its rows in order and the first throw ends it, so every row after the failing one is unrun — and a fixture that fails at its third row tells you nothing whatever about its tenth. Clearing one red routinely reveals the next one behind it, which is progress rather than a regression, and a run's red count falling by one while a new name appears is the normal shape of that progress. Say which it is when reporting. `VerifyResponsiveFollowing` stopped aborting on 15 September 2026, after the intent-region commit put four of its rows red and only the first was seen: it runs every row, prints each `GREEN` or `RED`, and fails afterwards if any did, which is the shape the newer fixtures (`VerifyCompanyIsTheFallback`, `VerifyAccompanyingThePlayer`, `VerifyCourtesy`) already had.
...
--candidate-funnel     every candidate a preparation refused, named with its stage — retiring with the chooser, see below
...
Three consequences bind whoever reads a number or a red here:
...
- **Every wall-clock threshold in this suite is judged in a regime no row declares**, so a per-frame cost row tests how far through the run it happens to sit as much as it tests the code beneath it. That is the shape of `AIC-425`, a crowd-cost row that disagreed threefold with itself on one tree, and it is plausibly this.
- **A lone red on either flaking row is more likely the harness than the change in front of it**, and a single green whole-suite run is correspondingly weak evidence that nothing broke. The two rows are `ore work breaks ore without excavating ordinary terrain` (5 red of 12 in-suite on 21 September, 3 of 3 green alone) and `combat is admitted only where it can be executed` (2 red of 4 in-suite, green alone). Both pass standalone; neither is fixed.
