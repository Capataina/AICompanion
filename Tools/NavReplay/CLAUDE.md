...
├─ VerifyAnchorsAndEvade.cs      a hover anchor carried across a wall mid-wait, and a dodge between two shots over lava
├─ VerifyEvadeKeepsTheJob.cs     the captured slope pin flown through the movement boundary, a route skimming water, and a cornered body in water
├─ VerifyNavigatorKeepsMoving.cs a body flying while its search runs, and a finished search not run again
...
The self-test is a handful of ledger rows, each a property of the core, and `sh Tools/verify.sh` runs it after the ledger's own self-test. It runs with the millisecond allowances lifted, because a corpus verdict is an oracle only when it cannot depend on how busy the machine was; nothing here is about a deadline.
...
**A dodge bends the job's flight only where the job's own flight would meet harm, and never presses the body into a wall.** Four scenes, each driven through `CoordinateMovement` the way the brain tick drives it — the job's controls, then `Evade`, then the motor's law and the contact. The first is the third orb play's slope pin, the capture's plan dump at the stuck tick cropped verbatim into the row, with a predicate that is active and predicts no hit: the body must arrive, never be still for the session reader's still run and never touch the pool. The second is the same scene with a hit waiting a few pixels along the route, so the headings have to choose, and the one pressing into the slope must lose to one that moves. The third is a route descending past a pillar and running along a pool's surface, which must not be bent at all. The fourth is a body already in water with a shot closing and the job asking into it, which must still turn away. Each was shown red before its mechanism existed: at the commit before the change the first two sat still asking `-9,0` into the slope for their whole run, the third bent 101 ticks of a dry flight, and the fourth flew into the shot because every heading read as hit on its first wet tick. Every failure prints the ticks' job, ask, bend and position.
