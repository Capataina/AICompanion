...
- **Its sprite frame is 110 × 166 px**, six frames in a 110 × 996 texture (`Content/Images/NPC_4.xnb` **inside the game's own install**, not a path in this repository, decompressed with FNA's own `LzxDecoder`, and `Main.npcFrameCount[4]` is 6). Scanning opaque pixels row by row, the eyeball is one solid run from roughly row 72 to row 165 of the frame, at most 110 px wide; everything above that is several separate runs, which are the tendrils.
...
**The flashlight is how lighting looks.** While the companion is heading to light somewhere, it shines a soft cone of real light toward the dark places in the direction it is travelling and at the spot it is going to put a torch, and the torch goes in when it arrives. The beam is brightest at the companion and dims along its length to nothing at its reach, as a real torch beam does, so the Gathering lane's Flashlight reach lengthens the fade rather than moving a hard edge. The owner confirmed on 15 September 2026 that this is exactly how it should work. The light is added through `Lighting.AddLight`, so the player really sees into the dark, and it cannot stop the torch from being placed — but **not for the reason this file used to give**. It named a `ReadTransientLights.cs` that discounted each carried light from the darkness reading; that file and the whole discount model went on 16 September 2026 in `90755be`, because under a carried light the world's own light was then unknowable and was guessed at three times. The sense reads the world's light directly now (`Brain/Infrastructure/Observation/ReadWorldLight.cs`): everything anybody carries reaches the engine through `Lighting.AddLight` into a per-frame list that only the next `ProcessBlur` merges, so copying the working map while the state reads Blur and blurring the copy with the engine's own `LightMap.Blur` gives the light the world would show with nobody carrying anything. A flashlight therefore cannot enter the darkness reading **by construction**, with no discount step to get wrong, and the same is true of the player's held torch and of the companion's own.
