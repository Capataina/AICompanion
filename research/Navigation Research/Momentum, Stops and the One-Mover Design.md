...
## The experiment ran on 13 September 2026, and the graph failed its own pass line

Three of the five parts shipped and one was refused, and the refusal is the finding. Removing the walk's own jump (`fb7519d`) made 27 corpus routes faster by 1,831 ticks with none slower and took the follow harness's mislands from 13 to 8; the reactive floor was deleted (`5ce9bac`) because the clearance search beside it already answered the one case it existed for; and the replay tool now prints planner time per route (`cc1532e`), which established the baseline the graph was judged against: p50 6.3 ms, p95 346 ms, max 458 ms per route, with the worst route closing only 1,037 nodes, so planning cost here is the price of proving jump edges out of a node rather than the number of nodes opened.

The entry-speed graph was then built in full — a node keyed by tile and entry-speed class, every traversal proved from its class velocity with a landing class, cost in ticks, an admissible tick heuristic, route memory keyed on class, the from-rest brake removed — and measured. The class count came out at five (0, 1.77, 2.52, 3.08 and 3.50 px/tick), forced by a ramp probe against the real motor: with three classes a body at 1.77 walks one tile to 2.52, rounds back to 1.77, and the graph never leaves the slow class while the body accelerates away. On planning it cost 3.81× at p95 (346 → 1,318 ms per route; total 13.3 → 51.7 s), and the one-variable attribution put 3.18× of that on the classes themselves and 1.22× on tick pricing plus the weaker heuristic, with expansion counts unchanged. On execution it did worse than the graph it replaced: the follow harness went from 89 walked, 43 partial and 1 not to 76, 38 and 20, and mislands from 8 to 40, because the planner proved better routes than the body could walk. The trace names the mechanism: a body arriving at 1.49 px/tick files as *rest* because the next class up is 1.77, the drop out of that node is proven from a standstill, and over 18 ticks the real body travels about 25 px further and lands a tile and a half off. Rounding down under-promises and the body overshoots; rounding up would only trade overshoot for undershoot. The general constraint is that the class gap times the longest steered move has to stay inside the arrival slack, which needs roughly twice the classes on a graph already 3.9× dearer. Soundness needs a finer quantisation than performance can afford, and that is a property of the design rather than of this implementation. The code is kept on the branch `experiment/entry-speed-graph` (`355787b`) so the measurement can be re-run rather than rebuilt; the finding is also in `Companion/Brain/Infrastructure/Movement/RoutePlanning/CLAUDE.md`.

What survives of the design is parts three to five in spirit and the pass-line discipline in fact. The stops attributed to the from-rest family (76% of the 119 measured) are still there, because the brake, `StartsFromRest` and `SettledFor` all remain, and the next candidate has to attack them without putting speed into the node key: a motor-side answer (a step-down and a landing-aim that lets a proven drop or jump be taken at speed, with the brake kept only where the proof genuinely started from rest) is the one that costs the planner nothing, and it is the one the measurement now points at. Any future attempt at speed in the key is held to the same two rows, the per-route p95 and the follow harness's mislands, before it is judged on anything else.
