...
**19 September 2026 historical investigation:** [Proposal 05](<Decision Architecture/Retained Course Design Rationale.md>) recommends a retained, repairable course over concrete opportunities, with local consequence planning. The owner has selected the direction; [Implement the Retained Course Brain](<proposal/Implement the Retained Course Brain.md>) now owns the full V1 contract, folder migration and verification gates, and has been on the live tick since `0bb2c8a` on 21 September 2026; the proposal folder's own guide carries how much of it the tree has caught up with. Earlier 01–04 proposals are [archived research](<Historical Evidence/Archived Brain Proposals/CLAUDE.md>); optional extensions live under the active plan's Modules folder. The [six complete approach attacks](<Decision Architecture/Attack Six Complete Brain Architectures/CLAUDE.md>) preserve costs, counterexamples, sources and reopening conditions; the [reproducible probes](<Evaluation and Observability/Probes/CLAUDE.md>) separate raw captured evidence from mathematical toy results. This investigation read the latest fifty commit bodies at `b3a4ad94f`; older research below retains its own dated baseline. Do not repeat the survey before reading these returns.
...
├─ Historical Evidence/                  full commit coverage, pivotal conversations and archived proposals
│  └─ Archived Brain Proposals/          preserved 01–04 resources with content provenance
...
└─ proposal/                             one selected complete implementation plan
   └─ Modules/                          conditional later extensions, never missing V1 basics
```
...
## Earlier rankings explain the route to the selected design
