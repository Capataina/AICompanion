...
The first useful discussion is therefore: **when the companion notices worthwhile nearby work, is it allowed to hold a small intention that survives a brief interruption, while continually checking that the work remains relevant?** My preferred reading of the Expected Behaviour is yes. That reading allows utility, trees, state machines and limited planning to be compared on their merits, while still excluding remote errands, rigid scripts and commitment to obsolete work. If the owner intends “never holds a plan” literally, that is a substantive constraint to record before comparing the remaining space.
