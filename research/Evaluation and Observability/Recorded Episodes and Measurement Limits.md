...
[^reader]: `Tools/SessionReport/Checks/CheckTheFight.cs`, `HuntingHadAWeaponThatCouldReach`, baseline lines 292–325 as of the date this note was written. The branch tests `Contains("outside-reach")` before constructing a definitive universal range explanation. That check was a named skip for five schema versions after 0.40.0 renamed the columns it read, and on 22 September 2026 it was rewritten as `CombatHeldTargetsItsBinderCouldNotSee`, reading the course's per-domain `target_evidence` with the right-to-left parse this note asks for; the finding above is about the check as it stood, not the one that exists now.
