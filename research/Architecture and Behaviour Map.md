...
At this baseline, `NavNode` in [NavPath.cs](../Companion/Brain/Infrastructure/Movement/RoutePlanning/NavPath.cs) contains a tile and mobility-resource state. It does **not** contain the body's full pose and velocity. The full dynamic `BodyState` belongs to simulation and local control search. The README's movement-abilities row conflates these. The distinction matters: two bodies reaching the same tile with different velocities can require different controls even when the coarse route node is identical.
