...
| `Tools/EngineReplay/Combat/VerifyHuntProgress.cs`, `VerifyHuntAdmissibility.cs`, `VerifyFiringPosition.cs`, `VerifyCombatPurpose.cs`, `VerifyCombatActorMatrix.cs`, `VerifyEncounterContext.cs`, `VerifySafetyIsALayerOnTheJob.cs`, `VerifyKnockbackAwareness.cs`, `VerifyOfferValidity.cs` | rewritten against `FightEnemies`, each row keeping the behaviour it proved | A, E |
