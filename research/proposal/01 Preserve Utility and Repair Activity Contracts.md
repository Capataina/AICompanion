...
**This file is the implementation plan for Proposal 1, originally prepared against checkout `4755e3b` on 12 September 2026. Implementation is authorised and in progress; none of its packages is complete merely because the plan exists.** Recheck the implementation revision, dirty work, affected CLAUDE chains, current policies and relevant Slate records when continuing. The original research source baseline remains historical evidence; current code and executable checks establish which contracts have actually landed. The seven-activity target, shared-safety migration and final acceptance obligations remain binding throughout incremental implementation.
