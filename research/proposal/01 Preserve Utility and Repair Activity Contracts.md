...
| Assess imminent avoidance | CombatReflexes through shared consequence and movement queries | Proposed motion plus alternatives and viable terminal conditions | Judging only passive motion, then overriding a safe escape blindly |
...
**Coverage and exit:** E16 and complete acceptance matrix. Completion: measured bounded acceptance and a candid unresolved ledger, never universal Terraria correctness.
...
| Shared context | Observation/value owners: player_motion_window, activity_context, confidence/basis, time_apart, remaining_time_estimate, reunion_time/status, combined risk and resource forecasts | Compare changed circumstances across a continuation decision; screen edge is not the policy boundary |
