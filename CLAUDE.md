...
Two 13 September playtests of packaged 0.22.41 are in `Telemetry/` (10:57, 11:06) and in README Current Behaviour. Chopping on those runs is Mimic by the in-game setting. Expected Behaviour now includes overtaking, a comfortable space around the player whose pull grows with distance, danger that depends on how a creature actually arrives, and choosing a place by how much work sits there. Slope walking (AIC-212) is still open. Not built, and named as such: mastery movement abilities, chaining several jobs (Proposal 3), a projectile dodge that jumps. Route search still treats a closed door as a wall. The portable historical corpus remains non-green. These checks establish the recorded fixtures, not reliable movement through every live cave or compatibility with every mod.
