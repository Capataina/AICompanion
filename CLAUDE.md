...
A sixth recording is the newest README Current Behaviour carries: the 13:27 capture of 14 September on packaged 0.24.0, written there from the ledger's own play rows. Its verdict is negative and it is what every 0.25.0 lane was built against — the reader exits 1 on it with 26 definitive issues, the companion is behind the player by more than three tiles on most rows where he is moving at all, hunt reads as inherently unusable on roughly half its decisions where the truth is a search that ran out of budget, and a platform jump dies repeatedly on keep-company's own Hold issued in the air. Behind it, five more back README's Current Behaviour, newest first: 09:28 on 14 September on packaged 0.23.0, then 18:56 on 13 September on packaged 0.22.46, 16:32 on 0.22.45 as its cross-check, and 10:57 and 11:06 on 0.22.41, where chopping is Mimic by the in-game setting. `Telemetry/` is gitignored, so which of them a given clone actually holds varies, and README is the durable account of what they showed.
