...
**`build.txt` reads `0.23.0`, packaged on 14 September 2026 and played once, at 09:28 the same day** (README Current Behaviour carries the numbers). The version separates the two halves of the tree that 0.22.46 could not: everything the 18:56 capture condemned was in 0.22.46, and everything that answered it is in 0.23.0. The 09:28 verdict on that second half: the torch hold is fixed and the carried-light discount is live; lighting still places almost nothing, now starved by its own three-site budget on 79% of ticks (each site charged for a fresh bounded route search that answers Unknown, with no memory of an Unknown, so the same nearest sites are re-asked every fifteen ticks); one proven jump is refused hundreds of times without a fault because the edge carries the profile's nominal speed rather than the launch state the proof used and a second judge in the preparation re-predicts from the live state and freezes the body; and the follow pocket is sealed in the model because the candidate generator scans four tiles across where the player's own exit jump was six. Two root fixes are in flight on worktree branches as of 14 September: a proven step is performed exactly as proven or it faults, with the preparation's prefix search and freeze cache deleted; and no activity or interaction runs its own route search to answer a reach question, enforced by the navigation boundary script. The split:
