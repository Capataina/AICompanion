...
**The tree is 0.33.0 and the retained-course brain is wired and unplayed.** Sixty-odd commits since 17 September implemented the course model, its bindings, consequence forecasts and repair, plus a combat-stance rewrite that merged guarding and hunting into one segmented plan with timed uses; `0bb2c8a` on 21 September made it the brain. `CoordinateBrainTick` calls `DecideCourseEachTick` rather than `Chooser.Choose`, and the bound step supplies the position request rather than the activity's own `Execute` — which is the load-bearing half, because letting the activity re-choose its site would rebuild the post-grant second chooser the plan names for deletion.

**Wired is not validated, and the difference is thirteen rows.** The suite files 207 rows with 17 red, four of which predate the switch; the other thirteen are behaviour contracts that held under the family chooser and do not hold under the course, and not one of them is a crash. The comfortable reading is that they assert a retired system and should be rewritten. That reading is wrong for at least four of them: "a damageable hostile in reach takes the body off keeping company", "danger lifts combat over work", "an assistance trip goes and returns" and "a closed door is opened rather than treated as a wall" are behaviours README describes rather than chooser internals, and a brain that fails them is not good enough to be the only brain whatever its architecture. One is measured rather than argued: on the shared-eagerness scene combat wins 5 ticks of 30 with its plan priced at 3.476 over a front of 21, so the shot is seen and not taken. Adjudicating the thirteen one at a time is `AIC-437` and it comes before a playtest is worth running.

Two properties of the wiring are worth knowing before touching it. **An observation is frozen for the life of one decision rather than one tick**, because a travel query can cost more than a tick's leftover allowance and the first wiring discarded and restarted one every tick — measured as a drop discovered on every tick with the search stuck at `evaluated=0 exhausted=False` for ever. And **publication is checked against the model-extended snapshot**, not the one the decision started from, because every travel and motion answer is appended as it completes and `ReadyToPublish` refuses a proposal naming a fact its snapshot does not carry. The eleven legacy selection paths stay until this has been played, so that a defect in play can still be attributed to one brain or the other.

**The suite is most of the way back and the last five reds are five separate questions.** The last fully clean run was `30fef2b-20260917-103231.jsonl` on 17 September — 171 rows, nothing red — and the stretch after it carried sixteen reds, eleven sharing one cause: combat moved onto the brain tick's shared `DecisionWorkBudget`, and the harness's per-case reset cleared the allowance without installing one, so every fixture entering below the tick threw rather than asserting. Production was never exposed, because `Brain.Tick` opens exactly one allowance around everything. That is fixed (`5fd0b4f`, `09815f0`, `51cd2b9`) and a clean run at `807b90e` files 202 rows, 146 pass, six red, nothing skipped. The survivors are carried as `AIC-422` through `AIC-425`; one of them, the crowd-planning cost row, is not a regression at all — it is absent from the 17 September baseline and its two measurement regimes disagree threefold on one tree.

**The defect worth knowing before touching combat**: a committed fight is released on the next tick whenever its target moves faster than about 0.4 px/tick, which is roughly a quarter of a zombie's walking pace, because the re-flown shot intercepts nothing. Bisected to `03986f0`. The arithmetic says the predicted impact point should be invariant in the re-pricing tick and it measurably is not, so one of the forecast's terms is not what it appears to be; `AIC-422` carries the measured tolerance table and the three candidate terms to read first. This is a plausible mechanism behind the flicker capture `2026-09-18_16-35-26-353` measured as a combat median run of one tick over 780 runs.
