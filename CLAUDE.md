...
- It never teleports. A sealed companion remains where the world left it until a future rescue behaviour exists.
...
The companion has one repository home, including its brain, body, equipment, cargo and engine integrations. The first route expansion uses the actual body pose; execution validates the full movement and accepts preparation only when it enables that move. Physical failure survives destination churn and is revalidated against unannounced terrain changes. Shared regroup pressure makes optional excursions yield as returning becomes costly, while the hands remain independent. Projectile aiming uses native flight phases and checks the final launch after accuracy variation.

Automatic observation combines continuous body/player samples with timestamped decision, movement, native projectile, damage and pickup events plus rolling local terrain snapshots. SessionReport can read an unnarrated run, reports missing capture explicitly and separates observation from inferred cause. It cannot reconstruct uncaptured terrain or predict an enemy's unobserved future script.

EngineReplay validates ordinary companion adapter cases and liquid transitions against native NPC collision. The final 2026-09-09 portable corpus has 86 full arrivals, 29 partial ends and 16 incomplete cases, against 78/27/25 at the responsibility-only refactor; seven old scenarios and one newly captured entry are gained with no lost full arrivals. The full corpus therefore remains non-green. These checks establish the recorded fixtures, not reliable movement through every live cave or compatibility with every mod. The next acceptance is a recorded unnarrated playtest, and the remaining corpus cases stay open under the navigation roadmap.
