...
The first playtest read back against its own telemetry landed three lanes of repair. Target selection is two questions rather than one — whether a shot exists from here, and failing that whether a position the companion can reach has one — and an enemy that answers neither is not hunted. Hunting scores below proven work because it exists to find new ore rather than to kill things, so it can never outrank mining or gathering. A jump's arc is proven from the take-off the performer actually reaches by running in over the real floor, not from the profile's nominal speed, which is what made a jump admitted, proven and performed at three different speeds. A step the macro proof refuses before its first tick now prices its tile and is remembered, because a refusal the planner never learns from is re-offered for ever by every later plan.

Two mechanisms are deliberately absent and block real work: there is no record anywhere of ground already covered, which is what hunting-as-exploration needs and what the rule that a one-way edge may be taken only after the player has taken it has no memory behind.
