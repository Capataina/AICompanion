...
Behaviours compete by score each tick for companionship, combat, gathering and nearby work. Shared safety can suspend ordinary activity only to escape water or lava; avoiding a hit is a bend in whatever the body is already doing, so the job keeps the body while it dodges. Shooting is independent hands work, so it runs while the feet follow or dodge; the torch fills an otherwise free hand. Missions and player-directed sending were abandoned.
