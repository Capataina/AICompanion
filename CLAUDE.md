...
navigation      corner nodes where the circle fits, a clearance field, one resumable flood that is also
                ...
                momentum steering with a speed cap, a bend cap and a brake
...
**What the suite says, on 15 September at `c94c797`**: `sh Tools/verify.sh` prints "96 rows: 59 pass, 1 red, 1 skipped, 35 measure, 0 sealed" and exits 1. The red is real and it is the orb's first defect on a real recording. The world run replays the player's track from the water-pocket capture of 14 September through the saved world, and the orb reaches sixteen of the twenty places the player stood, the four misses on dry ground with the flood unfinished at each; the same command at `806a998` gives the same sixteen to the pixel, so the gear lane did not move it. A checkpoint under water or lava the orb has no immunity for is excused by name, and on this route that excuses nothing. The skip is the session reader's before-numbers, whose capture declares the walker's schema. One behaviour change rode in with the body and every sight consumer inherited it: line of sight is the game's single-tile walk rather than its three-row beam, because a body one row above a floor never has the beam along that floor.

**Nothing about the orb has been played.** Every capture README's Current Behaviour describes is of the walker; the newest, 13:27 on 14 September, is now the track the orb is measured against rather than an account of it. The acceptance play is the path the walker failed: go right, drop all the way down, come back to the statue, and the orb should follow throughout and float up to light the dark area.
