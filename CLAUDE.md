...
The responsibility refactor has migrated the former `DecisionMatrix`, `Actions`, `Aiming`, `Work`, `Debug`, and standalone companion motor boundaries into focused brain subsystems and a shared movement system. EngineReplay validates ordinary adapter cases and liquid transitions against native NPC collision; those checks do not establish gameplay competence. Navigation still needs real playtest evidence on difficult terrain; a replay is a headless diagnostic, not a substitute for that evidence.
