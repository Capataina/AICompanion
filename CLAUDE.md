...
**The tree is 0.30.54 and the retained-course brain is built but not wired.** Sixty-odd commits since 17 September implemented the course model, its bindings, consequence forecasts and repair, plus a combat-stance rewrite that merged guarding and hunting into one segmented plan with timed uses. None of the course work runs in a game: `CoordinateBrainTick.cs` still calls `Chooser.Choose`, so every course class is reachable only from fixtures. That wiring is the single largest remaining piece and is `AIC-418`.
