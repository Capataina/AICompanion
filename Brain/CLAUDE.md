...
**What the reflex stop means:** on a tick a reflex takes the body, nothing is scored, no action runs, the position request is stale and the navigator is not called. The hold lasts a few ticks, then a refractory period during which the reflex will not fire again, so an enemy that stays adjacent cannot hold the companion in an endless dodge while the chooser never gets to shoot. **The hands are not part of the stop**, because a reflex takes the feet: a dodge is a jump or a step, the arm is free, and a tick where something is close enough to hit the body is the tick most worth shooting on. So step 4h runs on a reflex tick too, with no action to consult, which is safe by construction — nothing was chosen, so nothing can be holding a tool.
...
Five consequences to read off that table. **Two actions are scaled past the top of the ordinary band, and they are a ladder rather than two independent numbers**, because an ordinary action scores in 0..1 and the running one keeps the commitment bonus, so an action that has to *interrupt* one must clear that product and not merely reach 1. Guard is scaled to clear a committed ordinary action and survive to clear a committed guard, so a shooter getting a line on the player pulls the companion off a hunt or a loot run, and a drowning or burning companion leaves even that; the two constants live beside each other in `Weights.cs` and move together, because raising one without the one above it silently loses a rung. Guard reaching only 1 was not a tuning miss but arithmetic, and it is why a following companion could stand holding a torch with the player's danger reading full. Survive does not keep the companion out of water either, because crossing a pool is priced in the navigator and this is the backstop for a crossing that runs longer than the breath. The working score of chop and mine is set above walk-with's ceiling while the player travels and above loot unless the loot is close and valuable, so a job continues while the player keeps working and stops once the job's memory of the player's last hit runs out; the mine memory is much longer than the chop memory, because a vein takes longer to clear than a tree takes to fell. Wander only wins when everything else is zero, which by design is "the player is standing still, nothing is around, the companion is inside the calm band", with one exception: while the brain says the body is stranded in a sealed pocket, walk-with is discounted and wander scores just under the combat ceiling, so the body walks the pocket instead of pressing the wall (`Actions/Companionship/`). And hunting carries two factors nothing else does, both of which are the same argument: a leash that falls to zero as the companion's distance from the player passes what a screen can hold, and its own skin, `1 − CompanionDanger` with a floor. Hunting is the opportunistic thing a player does when little else is going on, never a mode they enter, so the score has to shrink with the two things that make wandering off wrong — being far from him, and being in trouble itself.
