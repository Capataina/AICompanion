...
namespace AICompanion.Brain.BehaviourDiagnostics;
...
public sealed class BrainTelemetry : ModSystem
{
    ...
    public static void Record(CompanionNPC companion)
    {
        ...
        sb.Append('\t').Append(brain.SensesMs.ToString("0.00")).Append('\t').Append(brain.ReflexMs.ToString("0.00"))
          ...
          .Append('\t').Append(SharedMovementSystem.AStar.CachedTiles)
        ...
        if (brain.Navigator.LastEdge is SharedMovementSystem.EdgeReport edge)
    ...
    private static string PlayerActivity(Player player, WorldObservation.Senses senses)
