...
public static class BehaviourCensus
{
    ...
    private static readonly int[,] faults = new int[Kinds, 5];
    ...
    public static string Report()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("  move            planned    begun completed  faulted   faults by reason");
        for (int k = 0; k < Kinds; k++)
        {
            ...
            sb.AppendLine($"  {((MoveKind)k).ToString(),-14}{planned[k],8:n0} {begun[k],8:n0} {completed[k],8:n0} {faulted[k],8:n0}   {(reasons.Count == 0 ? "-" : string.Join(", ", reasons))}");
        }
        sb.AppendLine();
